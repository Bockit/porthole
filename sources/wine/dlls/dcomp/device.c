...
/* IDCompositionDevice3 — Windows-only header type with no Wine IDL. CEF probes
 * for this interface right after DCompositionCreateDevice3; without S_OK it
 * abandons DComp entirely. The v3 interface only adds effect-factory methods
 * CEF never calls in this probe pattern, so returning the v2 vtable suffices. */
DEFINE_GUID(IID_IDCompositionDevice3, 0x0987cb06, 0xf916, 0x48bf,
            0x8d, 0x35, 0xce, 0x76, 0x41, 0x78, 0x1b, 0xd9);
...
static HRESULT do_composite(const struct composition_target *target, struct composition_visual *visual)
{
    ...
    if (visual->content)
    {
        if (IsEqualGUID(&visual->content_iid, &IID_IDXGISwapChain1))
        {
            ...
            /* Upstream uses BufferCount - 1 (flip-sequential front buffer). DXVK on macOS
             * has only one buffer and rejects any index > 0 with DXGI_ERROR_UNSUPPORTED,
             * so use index 0 — it's the current back buffer where DXVK draws the latest
             * frame. */
